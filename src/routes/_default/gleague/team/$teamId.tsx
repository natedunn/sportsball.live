...
function GleagueTeamPage() {
  ...
  const { data: convexTeamStats = [] } = useQuery(convexTeamStatsQueryOptions("gleague"));

  // Fetch all games for rolling averages and season trends
  const { data: recentGames = [] } = useQuery({
    ...teamRecentGamesQueryOptions("gleague", overview?.id ?? "", 100),
    enabled: !!overview?.id,
  });

  // Fetch Convex player stats (has GP, 3P%, etc. not available from roster endpoint)
  const { data: convexPlayerStats = [] } = useQuery({
    ...playerStatsByTeamQueryOptions("gleague", overview?.id ?? ""),
    enabled: !!overview?.id,
  });
