...
export const upsertTeam = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		if (existing) {
			...
			return await ctx.db.insert("wnbaTeam", data);
		}
	},
});
...
export const upsertGameEvent = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		if (existing) {
			...
			return await ctx.db.insert("wnbaGameEvent", data);
		}
	},
});
...
export const upsertTeamEvent = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		if (existing) {
			...
			return await ctx.db.insert("wnbaTeamEvent", data);
		}
	},
});
...
export const upsertPlayerEvent = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		if (existing) {
			...
			return await ctx.db.insert("wnbaPlayerEvent", data);
		}
	},
});

// Recalculate seasonal averages for a team from all wnbaTeamEvent rows
...
// Recalculate seasonal averages for a player from all wnbaPlayerEvent rows
